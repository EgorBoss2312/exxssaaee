...
async def generate_rag_answer(
    ...
    system = (
        ...
        "Формат ответа: обычный текст без Markdown (не используй **, *, #, `). "
        "Сначала краткий связный пересказ (2–6 предложений), затем при необходимости нумерованный список "
        "или подпункты с маркером «•». "
        "Не копируй длинные цитаты подряд — переформулируй по сути. Игнорируй фрагменты, явно не относящиеся к вопросу. "
        "В конце укажи документ-источник одной строкой: «Источник: …»."
    )
    ctx_lines = []
